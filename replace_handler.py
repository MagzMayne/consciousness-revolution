...
# The new enhanced handler with cssVarMap
new_handler = '''
                ...
                case 'dashboard_edit':
                    ...
                    if (context?.canEdit === true && context?.file) {
                        ...
                        // Detect which CSS variable to change (try multiple names for compatibility)
                        const cssVarMap = {
                        ...
                        if (cssVarOptions && newColor) {
                            ...
                            if (readResult.success) {
                                ...
                                // Try each CSS variable name until one matches
                                ...
                                if (foundVar) {
                                    ...
                                    // Write the file
                                    const writeResult = await araraFileOperation('write', context.file, fileContent, `Dashboard edit: ${propertyName} to ${newColor}`);
                                    ...
                                    if (writeResult.success) {
                                        ...
                                        abilityContext = `\\n\\n[FILE EDITED]: Changed ${propertyName} to ${newColor} in ${context.file}!\\n\\nLEARN THIS: CSS variable ${foundVar} controls this. Refresh to see!`;
                                    } else {
                        ...
                        // FALLBACK: Dashboard config path
...
old_pattern = r"// === DASHBOARD CUSTOMIZATION HANDLER \(Scalable 1000x\) ===\s*\n\s*case 'dashboard_edit':.*?break;\s*\n(?=\s*// === SELECTIVE MERGE)"
new_content = re.sub(old_pattern, new_handler.strip() + '\n\n', content, flags=re.DOTALL)
