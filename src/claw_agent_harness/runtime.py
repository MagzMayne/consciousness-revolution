...
class PortRuntime:
    ...
    def _score(tokens: set[str], module: PortingModule) -> int:
        # Base scoring: 1 point per matching token found in name, source_hint, or responsibility.
        # Bonus: +1 for exact name match, to slightly favour precise hits without disrupting ranking.
        name_lower = module.name.lower()
        haystacks = [name_lower, module.source_hint.lower(), module.responsibility.lower()]
        score = 0
        for token in tokens:
            if any(token in haystack for haystack in haystacks):
                score += 1
                if token == name_lower:
                    score += 1  # bonus for exact name match
        return score
