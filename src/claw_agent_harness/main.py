...
from .replLauncher import build_repl_banner, launch_repl
from .runtime import PortRuntime
from .session_store import load_session
from .setup import run_setup
from .skills.runner import build_default_skill_runner
from .tool_pool import assemble_tool_pool
from .tool_runners import run_bash, run_file_edit, run_file_read, run_file_write, run_glob, run_grep
...
def build_parser() -> argparse.ArgumentParser:
    ...
    subparsers.add_parser('repl-banner', help='print the REPL launch banner')
    subparsers.add_parser('plugins', help='list discovered plugins')
    subparsers.add_parser('skills', help='list registered skill definitions')
    ...
    # Real tool execution commands
    bash_parser = subparsers.add_parser('exec-bash', help='execute a shell command via BashTool')
    bash_parser.add_argument('cmd', help='shell command to run')
    bash_parser.add_argument('--timeout', type=int, default=30, help='timeout in seconds (default: 30)')
    bash_parser.add_argument('--cwd', default=None, help='working directory')

    file_read_parser = subparsers.add_parser('exec-file-read', help='read a file via FileReadTool')
    file_read_parser.add_argument('path', help='path to the file to read')
    file_read_parser.add_argument('--start-line', type=int, default=None)
    file_read_parser.add_argument('--end-line', type=int, default=None)

    file_write_parser = subparsers.add_parser('exec-file-write', help='write content to a file via FileWriteTool')
    file_write_parser.add_argument('path', help='path to the file to write')
    file_write_parser.add_argument('content', help='content to write')

    file_edit_parser = subparsers.add_parser('exec-file-edit', help='edit a file via FileEditTool (unique string replacement)')
    file_edit_parser.add_argument('path', help='path to the file to edit')
    file_edit_parser.add_argument('old_string', help='exact string to replace (must appear exactly once)')
    file_edit_parser.add_argument('new_string', help='replacement string')

    grep_parser = subparsers.add_parser('search-code', help='search for a pattern in files via GrepTool')
    grep_parser.add_argument('pattern', help='regex pattern to search for')
    grep_parser.add_argument('--path', default='.', help='directory or file to search in (default: .)')
    grep_parser.add_argument('--ignore-case', action='store_true')
    grep_parser.add_argument('--glob', default=None, dest='include_glob', help='filter files by glob pattern (e.g. *.py)')
    grep_parser.add_argument('--context', type=int, default=0, dest='context_lines', help='lines of context around matches')

    glob_parser = subparsers.add_parser('find-files', help='find files matching a glob pattern via GlobTool')
    glob_parser.add_argument('pattern', help='glob pattern (e.g. *.py)')
    glob_parser.add_argument('--path', default='.', help='directory to search in (default: .)')

    run_skill_parser = subparsers.add_parser('run-skill', help='run a registered skill by name')
    run_skill_parser.add_argument('name', help='skill name')
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == 'exec-tool':
        ...
        print(result.message)
        return 0 if result.handled else 1
    if args.command == 'exec-bash':
        result = run_bash(args.cmd, timeout=args.timeout, cwd=args.cwd)
        print(result.output or result.error)
        return 0 if result.success else 1
    if args.command == 'exec-file-read':
        result = run_file_read(args.path, start_line=args.start_line, end_line=args.end_line)
        print(result.output if result.success else result.error)
        return 0 if result.success else 1
    if args.command == 'exec-file-write':
        result = run_file_write(args.path, args.content)
        print(result.output if result.success else result.error)
        return 0 if result.success else 1
    if args.command == 'exec-file-edit':
        result = run_file_edit(args.path, args.old_string, args.new_string)
        print(result.output if result.success else result.error)
        return 0 if result.success else 1
    if args.command == 'search-code':
        result = run_grep(args.pattern, path=args.path, ignore_case=args.ignore_case, include_glob=args.include_glob, context_lines=args.context_lines)
        print(result.output if result.success else result.error)
        return 0 if result.success else 1
    if args.command == 'find-files':
        result = run_glob(args.pattern, path=args.path)
        print(result.output if result.success else result.error)
        return 0 if result.success else 1
    if args.command == 'run-skill':
        runner = build_default_skill_runner()
        run_result = runner.run(args.name)
        print(run_result.as_text())
