...
from src.claw_agent_harness.commands import PORTED_COMMANDS
from src.claw_agent_harness.parity_audit import run_parity_audit
from src.claw_agent_harness.port_manifest import build_port_manifest
from src.claw_agent_harness.query_engine import QueryEnginePort
from src.claw_agent_harness.tools import PORTED_TOOLS
...
class PortingWorkspaceTests(unittest.TestCase):
    ...
    def test_cli_summary_runs(self) -> None:
        result = subprocess.run(
            [sys.executable, '-m', 'src.claw_agent_harness.main', 'summary'],
            check=True,
            capture_output=True,
            text=True,
        )
    ...
    def test_parity_audit_runs(self) -> None:
        result = subprocess.run(
            [sys.executable, '-m', 'src.claw_agent_harness.main', 'parity-audit'],
            check=True,
            capture_output=True,
            text=True,
        )
    ...
    def test_commands_and_tools_cli_run(self) -> None:
        commands_result = subprocess.run(
            [sys.executable, '-m', 'src.claw_agent_harness.main', 'commands', '--limit', '5', '--query', 'review'],
            check=True,
            capture_output=True,
            text=True,
        )
        tools_result = subprocess.run(
            [sys.executable, '-m', 'src.claw_agent_harness.main', 'tools', '--limit', '5', '--query', 'MCP'],
            check=True,
            capture_output=True,
            text=True,
        )
    ...
    def test_subsystem_packages_expose_archive_metadata(self) -> None:
        from src.claw_agent_harness import assistant, bridge, utils
    ...
    def test_route_and_show_entry_cli_run(self) -> None:
        route_result = subprocess.run(
            [sys.executable, '-m', 'src.claw_agent_harness.main', 'route', 'review MCP tool', '--limit', '5'],
            check=True,
            capture_output=True,
            text=True,
        )
        show_command = subprocess.run(
            [sys.executable, '-m', 'src.claw_agent_harness.main', 'show-command', 'review'],
            check=True,
            capture_output=True,
            text=True,
        )
        show_tool = subprocess.run(
            [sys.executable, '-m', 'src.claw_agent_harness.main', 'show-tool', 'MCPTool'],
            check=True,
            capture_output=True,
            text=True,
        )
    ...
    def test_bootstrap_cli_runs(self) -> None:
        result = subprocess.run(
            [sys.executable, '-m', 'src.claw_agent_harness.main', 'bootstrap', 'review MCP tool', '--limit', '5'],
            check=True,
            capture_output=True,
            text=True,
        )
    ...
    def test_exec_command_and_tool_cli_run(self) -> None:
        command_result = subprocess.run(
            [sys.executable, '-m', 'src.claw_agent_harness.main', 'exec-command', 'review', 'inspect security review'],
            check=True,
            capture_output=True,
            text=True,
        )
        tool_result = subprocess.run(
            [sys.executable, '-m', 'src.claw_agent_harness.main', 'exec-tool', 'MCPTool', 'fetch resource list'],
            check=True,
            capture_output=True,
            text=True,
        )
    ...
    def test_setup_report_and_registry_filters_run(self) -> None:
        setup_result = subprocess.run(
            [sys.executable, '-m', 'src.claw_agent_harness.main', 'setup-report'],
            check=True,
            capture_output=True,
            text=True,
        )
        command_result = subprocess.run(
            [sys.executable, '-m', 'src.claw_agent_harness.main', 'commands', '--limit', '5', '--no-plugin-commands'],
            check=True,
            capture_output=True,
            text=True,
        )
        tool_result = subprocess.run(
            [sys.executable, '-m', 'src.claw_agent_harness.main', 'tools', '--limit', '5', '--simple-mode', '--no-mcp'],
            check=True,
            capture_output=True,
            text=True,
        )
    ...
    def test_load_session_cli_runs(self) -> None:
        ...
        result = subprocess.run(
            [sys.executable, '-m', 'src.claw_agent_harness.main', 'load-session', session_id],
            check=True,
            capture_output=True,
            text=True,
        )
    ...
    def test_tool_permission_filtering_cli_runs(self) -> None:
        result = subprocess.run(
            [sys.executable, '-m', 'src.claw_agent_harness.main', 'tools', '--limit', '10', '--deny-prefix', 'mcp'],
            check=True,
            capture_output=True,
            text=True,
        )
    ...
    def test_turn_loop_cli_runs(self) -> None:
        result = subprocess.run(
            [sys.executable, '-m', 'src.claw_agent_harness.main', 'turn-loop', 'review MCP tool', '--max-turns', '2', '--structured-output'],
            check=True,
            capture_output=True,
            text=True,
        )
    ...
    def test_remote_mode_clis_run(self) -> None:
        remote_result = subprocess.run([sys.executable, '-m', 'src.claw_agent_harness.main', 'remote-mode', 'workspace'], check=True, capture_output=True, text=True)
        ssh_result = subprocess.run([sys.executable, '-m', 'src.claw_agent_harness.main', 'ssh-mode', 'workspace'], check=True, capture_output=True, text=True)
        teleport_result = subprocess.run([sys.executable, '-m', 'src.claw_agent_harness.main', 'teleport-mode', 'workspace'], check=True, capture_output=True, text=True)
    ...
    def test_flush_transcript_cli_runs(self) -> None:
        result = subprocess.run(
            [sys.executable, '-m', 'src.claw_agent_harness.main', 'flush-transcript', 'review MCP tool'],
            check=True,
            capture_output=True,
            text=True,
        )
    ...
    def test_command_graph_and_tool_pool_cli_run(self) -> None:
        command_graph = subprocess.run([sys.executable, '-m', 'src.claw_agent_harness.main', 'command-graph'], check=True, capture_output=True, text=True)
        tool_pool = subprocess.run([sys.executable, '-m', 'src.claw_agent_harness.main', 'tool-pool'], check=True, capture_output=True, text=True)
    ...
    def test_execution_registry_runs(self) -> None:
        from src.claw_agent_harness.execution_registry import build_execution_registry
    ...
    def test_bootstrap_graph_and_direct_modes_run(self) -> None:
        graph_result = subprocess.run([sys.executable, '-m', 'src.claw_agent_harness.main', 'bootstrap-graph'], check=True, capture_output=True, text=True)
        direct_result = subprocess.run([sys.executable, '-m', 'src.claw_agent_harness.main', 'direct-connect-mode', 'workspace'], check=True, capture_output=True, text=True)
        deep_link_result = subprocess.run([sys.executable, '-m', 'src.claw_agent_harness.main', 'deep-link-mode', 'workspace'], check=True, capture_output=True, text=True)
