 ...
 * PATENT DECLARATION:
 * File: game.js
 * Declaration ID: IP-BA96F3B-MLL28ZVQ
 * Date: 2026-02-13
...
function initGame() {
  ...
  // ============================================
  // Mission Banner Interactive Features
  // ============================================
  
  const MISSION_BANNER_STORAGE_KEY = 'freedomQuest_missionBannerMinimized';
  const ACTIVE_LAYER_CLASS = 'active';
  
  function initMissionBanner() {
    const banner = document.getElementById('mission-banner');
    const toggleBtn = document.getElementById('mission-banner-toggle');
    ...
    try {
      const isMinimized = localStorage.getItem(MISSION_BANNER_STORAGE_KEY) === 'true';
      if (isMinimized) {
      ...
      console.warn('Failed to restore mission banner state:', e);
    }
    
    // Toggle banner on button click
    toggleBtn.addEventListener('click', () => {
      ...
      try {
        localStorage.setItem(MISSION_BANNER_STORAGE_KEY, isMinimized.toString());
      } catch (e) {
        console.warn('Failed to save mission banner state:', e);
      }
      
      // Show notification
      showNotification(
        isMinimized ? 'Mission banner minimized' : 'Mission banner expanded',
        2000
      );
    });
  }
  ...
  // ============================================
  // Global Awareness Legend Interactive Features
  // ============================================
  
  function initGlobalAwarenessLegend() {
    const legendLayers = document.querySelectorAll('.legend-layer');
    
    if (!legendLayers || legendLayers.length === 0) return;
    
    const layerNotifications = {
      disasters: {
        title: 'Natural Disasters',
        message: 'NASA EONET tracks natural disasters worldwide including wildfires, storms, and volcanic activity. Click EONET button in NASA Toolkit to visualize.'
      },
      cooperation: {
        title: 'International Cooperation',
        message: 'Zones of active international collaboration promote shared scientific research and peaceful resource management.'
      },
      humanitarian: {
        title: 'Humanitarian Aid Zones',
        message: 'Active international assistance zones providing emergency relief, medical care, and support to communities in need.'
      },
      peace: {
        title: 'Peace Initiatives',
        message: 'Active peacekeeping missions and peace-building initiatives fostering dialogue, reconciliation, and conflict resolution worldwide.'
      }
    };
    ...
    legendLayers.forEach((layer) => {
      layer.addEventListener('click', () => {
        ...
        // Toggle active state
        const wasActive = layer.classList.contains(ACTIVE_LAYER_CLASS);
        
        // Remove active from all layers
        legendLayers.forEach(l => l.classList.remove(ACTIVE_LAYER_CLASS));
        
        // Add active to clicked layer if it wasn't already active
        if (!wasActive) {
          layer.classList.add(ACTIVE_LAYER_CLASS);
          
          // Show educational notification
          const notification = layerNotifications[layerType];
          if (notification) {
            showNotification(
          ...
          // Special handling for disasters layer - suggest EONET
          if (layerType === 'disasters') {
            setTimeout(() => {
              showNotification('Try the EONET button in NASA Toolkit to see live disaster data!', 4000);
            }, 6500);
          }
        } else {
          showNotification('Data layer deactivated', 2000);
        }
      });
    });
  }
